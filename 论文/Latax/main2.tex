...
% In this section, we discuss the pipeline for the generation of self-captured data, personalization process consisting of seven steps, the network architecture, the training process and two technological innovations of the current research project – subject-aware sampling and two-stage personalization process.
...
%     \tikzstyle{stage}=[gap2block, fill=blue!7, minimum width=22mm,
%                        minimum height=11mm, font=\footnotesize]
%     \tikzstyle{contrib}=[stage, fill=orange!12, draw=orange!70!black]
%     \tikzstyle{outp}=[stage, fill=green!10, draw=green!60!black]

%     \node[stage] (cap)     {\texttt{capture.py}\\\tiny webcam};
%     \node[stage, right=of cap] (prelab) {\texttt{prelabel.py}\\\tiny weak labels};
%     \node[stage, right=of prelab] (review){\texttt{review\_app.py}\\\tiny manual review};
%     \node[stage, right=of review] (gather){\texttt{gather.py}\\\tiny merge users};

%     \node[stage, below=8mm of cap, xshift=11mm]    (mtcnn) {\texttt{face\_crop.py}\\\tiny MTCNN};
%     \node[stage, right=of mtcnn] (merge) {\texttt{merge\_ferplus.py}\\\tiny + FER+, RAF-DB};
%     \node[contrib, right=of merge] (train) {\texttt{train.py}\\\tiny generic + sampler};
%     \node[contrib, right=of train] (ft)    {\texttt{finetune.py}\\\tiny personalization};

%     \draw[gap2arr] (cap)    -- (prelab);
%     \draw[gap2arr] (prelab) -- (review);
%     \draw[gap2arr] (review) -- (gather);
%     \draw[gap2arr] (gather.south) |- (mtcnn.north);
%     \draw[gap2arr] (mtcnn)  -- (merge);
%     \draw[gap2arr] (merge)  -- (train);
%     \draw[gap2arr] (train)  -- (ft);

%     \node[outp, below=11mm of train] (bestpt) {\texttt{best.pt}};
%     \node[outp, below=11mm of ft]    (perspt) {\texttt{personalized.pt}};
%     \draw[gap2arr, dashed] (train.south) -- (bestpt.north);
%     \draw[gap2arr, dashed] (ft.south)    -- (perspt.north);
%   \end{tikzpicture}}
...
% The remaining three steps involve preparation of the training set and training of the network. The script face crop.py applies MTCNN on all frames from self-captured dataset and saves the tight crop of 224 x 224 size in a separate directory tree (raw face/). The script merge ferplus.py merges the self-captured subset with FER+ and RAF-DB datasets into three CSV files (dataset/train.csv, dataset/val.csv and dataset/test.csv). The script train.py loads these files, applies subject-aware sampler to the dataset and trains the ResNet-18 network with saving the best checkpoint by validation accuracy. Finally, the script finetune.py loads generic checkpoint, makes personalization step on a target subject and saves the personalized checkpoint.
